...
We also wanat to make sure R knows that the years are numbers, not text ("character" class) data. We can use `as.numeric()` to force this.

```{r}
bnia_lib_year <- bnia_lib_year %>% mutate(year = as.numeric(year))
```

The "value is the number of people with a library card per 1,000 residents. Let's make that a percentage.
...
Try changing the neighborhoods on line 51 and running the code again.

Instead of grouping by "year" on line 117, try grouping by Community Statistical Area ("CSA2020").
